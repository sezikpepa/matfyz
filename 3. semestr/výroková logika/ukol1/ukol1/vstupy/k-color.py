...
def generate_cnf(vertices, edges, k):
	...
	#every vertex has at least one color, vytváříme klauzule, kde vrchol spojujeme se všemi možnými barvami, pokud by nebyla vrcholu přiřazena barva, tak nebude klauzule platit
	...
	#every vertex has maximum one color, postupně spojujeme všechno vrcholy s dvojicemi barev. Pokud by vrchol měl dvě různé barvy, pokud by tato situace nastala, tak pomocí negace dostaneme situaci 0 v 0 a tím pádem 
	#SAT solver vrátí neplatnost formule
	for vertex in vertices:		
	...
	#connected verticies do not have the same color. Analogická situace jako v předchozím kroku, pouze se změnou, že testujeme dvojice barva-vrchol s barva-vrchol, u těch které jsou spojené hranou. Pokud jsou barvy stejné
	#SAT solver odpoví neplatnost
