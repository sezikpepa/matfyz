...
class Net_inserter:
    ...
    def color_input(self, mouse_position: Tuple) -> None:
        if not isinstance(mouse_position, Tuple):
            raise TypeError(f"ERROR: variable mouse_position has to be Tuple -> {mouse_position}")
        if len(mouse_position) != 2:
            raise ValueError(f"ERROR: variable mouse_position has to have two elements inside -> {mouse_position}")
        if mouse_position[0] < 0:
            raise ValueError(f"ERROR: mouse_position[0] can not be negative -> {mouse_position[0]}")
        if mouse_position[1] < 0:
            raise ValueError(f"ERROR: mouse_position[1] can not be negative -> {mouse_position[1]}")
    ...
    def box_clicked(self, mouse_position: Tuple):
        if not isinstance(mouse_position, Tuple):
            raise TypeError(f"ERROR: variable mouse_position has to be Tuple -> {mouse_position}")
        if len(mouse_position) != 2:
            raise ValueError(f"ERROR: variable mouse_position has to have two elements inside -> {mouse_position}")
...
class Info_window:
    def __init__(self, x, y, width, height, color, text, hide):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
    ...
    def clicked_check(self, mouse_position: Tuple[int]):
        if not isinstance(mouse_position, Tuple):
            raise TypeError(f"ERROR: variable mouse_position has to be tuple -> {mouse_position}")
        if len(mouse_position) != 2:
            raise TypeError(f"ERROR: variable mouse_position has to have two elements -> {mouse_position}")
...
class Algorithm_helper:
    def __init__(self, x, y, width, height, color, algorithm):
        if not isinstance(x, int):
            raise TypeError(f"ERROR: variable x has to be int -> {x}")
        if not isinstance(y, int):
            raise TypeError(f"ERROR: variable y has to be int -> {y}")
    ...
    def check_for_shorter(self) -> None:
        changed = True
        while changed:
            changed = False
            if len(self.moves) >= 2:
                ...
                elif self.moves[-1][-1] == "2" and self.moves[-2][-1] != "2" and (self.moves[-1][0] not in "xyz" and self.moves[-2][0] not in "xyz") and (self.moves[-1][0] == self.moves[-2][0]):
