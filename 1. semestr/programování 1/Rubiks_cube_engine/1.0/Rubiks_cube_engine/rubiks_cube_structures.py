...
from rubiks_cube_nets import solved_cube_net, cross_on_yellow_net, cross_on_white_net, corners_solved_net, first_two_layers_solved_net, oll_dot_net, oll_solved_net
...
class Rubiks_cube_net:
    ...
    def solved_check(self) -> bool:
        return self.faces == solved_cube_net
...
class Rubiks_cube: 
    ...
    def __init__(self, shift: float, net_scale: int, net_x: int, net_y: int, mode:int = None) -> None:
        if not isinstance(shift, (int, float)):
